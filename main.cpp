...
class   T_path
{
  ...
  T_towns     free_towns_;
public:
  T_path(T_matr const &dist_matr):dist_matr_(dist_matr)
  ,shortest_path_len_(std::numeric_limits< T_dist >::max()) {
...
private:
  void push_town_with_delta_dist(T_town  town, T_dist  delta_dist = 0) {
  ...
  bool successfully_push_good_min_free_town_not_less_than( T_town town_start) {
    return std::find_if (free_towns_.lower_bound(town_start), free_towns_.end (), 
      [&] (auto town) {return this -> successfully_push_good_town(town);}) != free_towns_.end();
  }

  bool successfully_inc_back_town() {
    return path_cur_.size() > 1 && (successfully_push_good_min_free_town_not_less_than(pop_and_get_town() + 1) || successfully_inc_back_town());
  }
  ...
  T_town pop_and_get_town() {
    ...
    path_cur_len_ -= dist_matr_ [penultimate_town][back_town];
    return  back_town;
  }
